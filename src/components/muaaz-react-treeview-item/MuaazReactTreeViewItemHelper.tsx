...
import { CHECKBOX_STATE } from './MuaazReactTreeViewItemConsts'
import MuaazReactTreeView from '../muaaz-react-treeview/MuaazReactTreeView';
import { TreeItem } from './MuaazReactTreeViewItemProps';

const isSelected = (treeview: MuaazReactTreeView, item: TreeItem): boolean => {
  return treeview.state.selectedItem === item;
}

const  getClassItem = (treeview: MuaazReactTreeView, item: TreeItem, level: number, selectRow: boolean): string => {
    const classNames = ['muaaz-react-treeview-item-container'];
...
const getClassIcon = (treeview: MuaazReactTreeView, item: TreeItem): string => {
...
const getClassExpandButton = (treeview: MuaazReactTreeView, treeviewItem: any, item: TreeItem): string => {
...
const getClassCheckbox = (treeview: MuaazReactTreeView, item: TreeItem): string => {
...
const getClassIconAndText = (treeview: MuaazReactTreeView, item: TreeItem, selectRow: boolean): string => {
...
const getInnerText = (treeview: MuaazReactTreeView, item: TreeItem): JSX.Element => {
  if (treeview.props.actionButtons) {
      ...
      if (treeview.props.onRenderItem) {
          ...
          return item.text as any;
      }
  }
}

const treeviewItemContainerDoubleClick = (treeviewItemExpandButtonMouseDown: Function, treeview: MuaazReactTreeView, selectRow: boolean, event: React.MouseEvent): void => {
    /*
...
const treeviewItemContainerMouseDown = (treeview: MuaazReactTreeView, item: TreeItem, selectRow: boolean, event: React.MouseEvent): void => {
  if (treeview.state.selectedItem !== item) {
      ...
      if (treeview.props.onSelectItem) {
          const shouldFireEvent = (treeview.props.selectRow && target.classList.contains('muaaz-react-treeview-item-container')) ||
              (!treeview.props.selectRow && target.classList.contains('text-inner'))
...
const treeviewItemExpandButtonMouseDown = (treeview: MuaazReactTreeView, treeviewItem: any, item: TreeItem): void => {
  const conclusion = () => {
  ...
  if (item.expanded) {
      ...
      if (treeview.props.lazyLoad) {
        ...
        if (treeview.props.lazyLoad && treeview.props.onLazyLoad) {
            treeview.props.onLazyLoad(item, (children: TreeItem[]) => {
                treeview.api.loadData(treeview, children, item);
                conclusion();
            });
        } else {
            treeview.api.load(item).then((loadedData: any) => conclusion())
        }
...
const treeviewItemCheckboxMouseDown = (treeviewItem: any, item: TreeItem): void => {
...
const _checkNode = (treeviewItem: any, item: TreeItem): void => {
  item.state = CHECKBOX_STATE.CHECKED;
  _refreshCheckboxStateChildren(item);
  _refreshCheckboxStateParents(treeviewItem);
}

const _uncheckNode = (treeviewItem: any, item: TreeItem): void => {
  item.state = CHECKBOX_STATE.UNCHECKED;
  _refreshCheckboxStateChildren(item);
  _refreshCheckboxStateParents(treeviewItem);
}

const _refreshCheckboxStateChildren = (item: TreeItem): void => {
  if (item.children) {
      item.children.forEach(function (child: TreeItem) {
...
const _allItemsAreChecked = (items: TreeItem[]): boolean => {
...
const _allItemsAreUnchecked = (items: TreeItem[]): boolean => {
  for (let index = 0; index < items.length; index++) {
      const item = items[index];
...
const _refreshCheckboxStateParents = (treeviewItem: any): void => {
...
const _isChecked = (item: TreeItem): boolean => {
  return item.state === CHECKBOX_STATE.CHECKED;
}

const _isUnchecked = (item: TreeItem): boolean => {
